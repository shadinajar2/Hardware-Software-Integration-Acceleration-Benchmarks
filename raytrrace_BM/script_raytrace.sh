...
perf report --stdio > report_raytrace_baseline.txt
echo "[+] Baseline report saved to report_raytrace_baseline.txt"

echo "[*] --- 3. Flame Graph Generation ---"
# Convert perf.data into a flame graph SVG
perf script > out.perf
./FlameGraph/stackcollapse-perf.pl out.perf > out.folded
./FlameGraph/flamegraph.pl out.folded > raytrace_flamegraph.svg
echo "[+] Flame graph saved to raytrace_flamegraph.svg"

echo "[*] --- 4. Post-Optimization Execution & Comparison ---"
# Profile the optimized raytrace script
perf record -e cpu-clock -F 999 -g -- python3-dbg run_benchmark_opt.py

# Export the optimized performance data
perf report --stdio > report_raytrace_opt.txt
echo "[+] Optimized report saved to report_raytrace_opt.txt"

echo "[*] --- 5. Cleanup ---"
# Clean up temporary parsing files
rm out.perf out.folded
echo "[+] Raytrace execution complete! Compare the reports to see the performance delta."