...
class BitfieldBase(object):
    ...
    def __init__(self, x):
        if isinstance(x, BitfieldBase):
            ...
            import io
            # Buffer the input file to prevent massive system call overhead
            self.f = x if isinstance(x, io.BufferedReader) else io.BufferedReader(x, buffer_size=65536)
            self.bits = 0
            self.bitfield = 0x0
            self.count = 0
...
class HuffmanTable(object):
    # Reverted to original implementation: Python's pure C array iteration 
    # is faster here than allocating tuples to query a dictionary.
...
def move_to_front(l, c):
    # Optimized: O(1) array pop/insert without creating massive intermediate slice lists
    if c:
        l.insert(0, l.pop(c))


def bwt_transform(L):
    # Optimized: Histogram-based counting sort instead of O(N log N) sorted() + find()
    counts = [0] * 256
    for symbol in L:
        counts[symbol] += 1
...
def bwt_reverse(L, end):
    ...
    # Optimized: Pre-allocate a bytearray to prevent continuous resizing
    out = bytearray(n)
    for i in range(n):
        end = T[end]
        out[i] = L[end]
...
def decode_huffman_block(b, out):
    ...
    # Optimized: Using extend on bytearray rather than allocating temporary lists
...
def bzip2_main(input):
    ...
    # Optimized: Uses a bytearray directly
    out = bytearray()
    while True:
...
def gzip_main(field):
    ...
    # Optimized: Uses a bytearray and raw ints
    out = bytearray()
    while True:
